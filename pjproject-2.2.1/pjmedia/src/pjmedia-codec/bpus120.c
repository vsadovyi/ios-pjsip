...
static pj_status_t bpus120_open(pjmedia_codec *codec, pjmedia_codec_param *attr )
{
    ...
    opus_encoder_ctl(bpus120->enc, OPUS_SET_COMPLEXITY(5));
...
static pj_status_t  _bpus120_encode(pjmedia_codec *codec, const struct pjmedia_frame *input, unsigned output_buf_len, struct pjmedia_frame *output)
{
    ...
    int size1 = opus_encode(enc, input->buf, 480, output->buf+1, output_buf_len-1);
    buffer[0] = size1;
    int size2 = opus_encode(enc, input->buf+960, 480, output->buf+size1+2, output_buf_len - size1 - 2);
