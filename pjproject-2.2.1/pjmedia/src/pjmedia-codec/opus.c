...
static pj_status_t opus_dealloc_codec(pjmedia_codec_factory *factory,
                                      pjmedia_codec *codec);
static pj_status_t pjmedia_codec_opus_deinit();
...
static pjmedia_codec_op opus_op = {&opus_codec_init,   &opus_codec_open,
                                   &opus_codec_close,  &opus_codec_modify,
                                   &opus_codec_parse,  &opus_codec_encode,
                                   &opus_codec_decode, &opus_codec_recover};

/* Definition for OPUS codec factory operations. */
static pjmedia_codec_factory_op opus_factory_op = {
    &opus_test_alloc,  &opus_default_attr,  &opus_enum_codecs,
    &opus_alloc_codec, &opus_dealloc_codec, &pjmedia_codec_opus_deinit};
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_codec_opus_set_internal_clock_rate(unsigned clock_rate) {
...
/*
 * Modify codec settings. - Vlad's changes
 */
static pj_status_t opus_codec_modify(pjmedia_codec *codec,
                                     const pjmedia_codec_param *attr) {
  ...
  for (i = 0; i < attr->setting.enc_fmtp.cnt; ++i) {
    if (pj_stricmp(&attr->setting.enc_fmtp.param[i].name,
                   &STR_FMTP_MAX_AVERAGE_BITRATE) == 0) {
      int val = (int)(pj_strtoul(&attr->setting.enc_fmtp.param[i].val));
      if (val >= 6000 && val <= 510000) {
        opus_encoder_ctl(opus->psEnc, OPUS_SET_BITRATE(val));
      }
      break;
    }
  }
...
static pj_status_t opus_codec_encode(pjmedia_codec *codec,
  ...
  ret = opus_encode(opus->psEnc, (opus_int16 *)input->buf, nsamples,
...
#if _TRACE_OPUS
  PJ_LOG(4, (THIS_FILE,
             "Encoder packet size %d for input %d ouput max len %d @ %d",
             output->size, input->size, output_buf_len,
             (unsigned)output->timestamp.u64));
#endif
  return PJ_SUCCESS;
}
