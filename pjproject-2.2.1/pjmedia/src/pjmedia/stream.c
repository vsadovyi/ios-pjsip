...
static void trace_jb_get(pjmedia_stream *stream, pjmedia_jb_frame_type ft,
  ...
  if (!TRACE_JB_OPENED(stream)) return;

  /* Print timestamp. */
  if (trace_jb_print_timestamp(&p, endp - p)) goto on_insuff_buffer;
  ...
  if ((len < 0) || (len >= endp - p)) goto on_insuff_buffer;
  p += len;

  /* Print JB state */
  if (trace_jb_print_state(stream, &p, endp - p)) goto on_insuff_buffer;

  /* Print end of line */
  if (endp - p < 2) goto on_insuff_buffer;
  *p++ = '\n';
...
static void send_keep_alive_packet(pjmedia_stream *stream) {
  ...
  TRC_((stream->port.info.name.ptr, "Sending keep-alive (RTCP and empty RTP)"));
  ...
  pjmedia_transport_send_rtp(stream->transport, stream->enc->out_pkt, pkt_len);
  ...
  /* Send to RTCP port */
  pjmedia_transport_send_rtcp(stream->transport, stream->enc->out_pkt, pkt_len);
  ...
  for (samples_count = 0; samples_count < samples_required;
    ...
    if (frame_type == PJMEDIA_JB_MISSING_FRAME) {
      ...
      if (stream->codec->op->recover && stream->codec_param.setting.plc &&
        ...
        status = pjmedia_codec_recover(stream->codec, (unsigned)frame_out.size,
                                       &frame_out);
      ...
      if (stream->jb_last_frm != frame_type) {
        ...
        PJ_LOG(5, (stream->port.info.name.ptr,
                   "Jitter buffer empty (prefetch=%d)%s", jb_state.prefetch,
                   with_plc));
    ...
    if (frame_type == PJMEDIA_JB_NORMAL_FRAME) {
      ...
      if (frame_type == PJMEDIA_JB_MISSING_FRAME) {
        ...
        if (frame_type != stream->jb_last_frm) {
          ...
          pjmedia_jbuf_get_state(stream->jb, &jb_state);
          PJ_LOG(5, (stream->port.info.name.ptr,
                     "Jitter buffer empty (prefetch=%d)", jb_state.prefetch));
...
static pj_status_t send_rtcp(pjmedia_stream *stream, pj_bool_t with_sdes,
  ...
  if (with_sdes) {
    ...
    status = pjmedia_rtcp_build_rtcp_sdes(&stream->rtcp, pkt + len, &sdes_len,
                                          &sdes);
    if (status != PJ_SUCCESS) {
...
/* Build RTCP XR packet */
  ...
  if (with_xr) {
    ...
    pjmedia_rtcp_build_rtcp_xr(&stream->rtcp.xr_session, 0, &xr_pkt, &xr_len);
    ...
    if (xr_len + len <= max_len) {
      ...
      if (stream->rtcp_xr_dest_len) {
        pjmedia_transport_send_rtcp2(stream->transport, &stream->rtcp_xr_dest,
  ...
  if (with_bye) {
    ...
    if (status != PJ_SUCCESS) {
      PJ_PERROR(
          4, (stream->port.info.name.ptr, status, "Error generating RTCP BYE"));
    } else {
      len += (int)bye_len;
    }
  }
...
static void check_tx_rtcp(pjmedia_stream *stream, pj_uint32_t timestamp) {
  ...
  if (stream->rtcp_last_tx == 0) {
    ...
    status =
        send_rtcp(stream, !stream->rtcp_sdes_bye_disabled, PJ_FALSE, with_xr);
    if (status != PJ_SUCCESS) {
      PJ_PERROR(4, (stream->port.info.name.ptr, status, "Error sending RTCP"));
    }
...
static void rebuffer(pjmedia_stream *stream, pjmedia_frame *frame) {
  ...
  pj_assert(stream->enc_buf_count + (frame->size >> 1) < stream->enc_buf_size);
...
static pj_status_t put_frame_imp(pjmedia_port *port, pjmedia_frame *frame) {
  ...
  if (stream->use_ka) {
    ...
    dtx_duration =
        pj_timestamp_diff32(&stream->last_frm_ts_sent, &frame->timestamp);
    if (dtx_duration >
  ...
  if (stream->has_opus_support) rtp_ts_len = stream->rtp_opus_tx_ts_len_per_pkt;
  ...
  if (stream->tx_dtmf_count) {
    ...
    if (status != PJ_SUCCESS) {
      LOGERR_((stream->port.info.name.ptr, "Codec encode() error", status));
      return status;
    }
  ...
  if (status != PJ_SUCCESS) {
    LOGERR_((stream->port.info.name.ptr, "RTP encode_rtp() error", status));
    return status;
  }
  ...
  if (status != PJ_SUCCESS) {
    PJ_PERROR(4, (stream->port.info.name.ptr, status, "Error sending RTP"));
  }
 ...
 */
static void handle_incoming_dtmf(pjmedia_stream *stream, const void *payload,
                                 unsigned payloadlen) {
  ...
  if (payloadlen < sizeof(pjmedia_rtp_dtmf_event)) return;
  ...
  PJ_LOG(5, (stream->port.info.name.ptr, "Received DTMF digit %c, vol=%d",
             digitmap[event->event], (event->e_vol & 0x3F)));
   ...
   */
  if (stream->dtmf_cb) {
    stream->dtmf_cb(stream, stream->dtmf_cb_user_data, digitmap[event->event]);
...

{
  ...
  if (bytes_read < (pj_ssize_t)sizeof(pjmedia_rtp_hdr)) return;
  ...
  if (channel->paused) goto on_return;
  ...
  if (seq_st.status.value) {
    ...
    if (seq_st.status.flag.badpt) {
      PJ_LOG(4, (stream->port.info.name.ptr, "Bad RTP pt %d (expecting %d)",
  ...
  if (seq_st.status.flag.restart) {
    ...
    status = pjmedia_codec_parse(stream->codec, (void *)payload, payloadlen,
                                 &ts, &count, frames);
    if (status != PJ_SUCCESS) {
      LOGERR_((stream->port.info.name.ptr, "Codec parse() error", status));
      count = 0;
    }
    ...
    if (stream->has_g722_mpeg_bug) {
      if (stream->rtp_rx_check_cnt) {
        ...
        if (seq_st.diff == 1 && stream->rtp_rx_last_ts &&
            ts.u64 > stream->rtp_rx_last_ts && stream->rtp_rx_last_cnt > 0) {
          ...
          peer_frm_ts_diff = ((pj_uint32_t)ts.u64 - stream->rtp_rx_last_ts) /
    ...
    for (i = 0; i < count; ++i) {
      ...
      if (discarded) pkt_discarded = PJ_TRUE;
    }
  ...
  if (status != 0) {
    LOGERR_((stream->port.info.name.ptr, "Jitter buffer put() error", status));
    pkt_discarded = PJ_TRUE;
    goto on_return;
  }
...
on_return:
  ...
  if (stream->rtcp.received >= 10 && !stream->initial_rr) {
    status =
        send_rtcp(stream, !stream->rtcp_sdes_bye_disabled, PJ_FALSE, PJ_FALSE);
    if (status != PJ_SUCCESS) {
      PJ_PERROR(4, (stream->port.info.name.ptr, status,
...
static pj_status_t create_channel(pj_pool_t *pool, pjmedia_stream *stream,
  ...
  if (param->type == PJMEDIA_TYPE_AUDIO) {
    ...
    if (channel->out_pkt_size >
      ...
      channel->out_pkt_size = PJMEDIA_MAX_MTU - PJMEDIA_STREAM_RESV_PAYLOAD_LEN;
    }
  } else {
    return PJ_ENOTSUP;
  }
...

{
  ...
  if (pool == NULL) {
    own_pool = pjmedia_endpt_create_pool(endpt, "strm%p", PJMEDIA_STREAM_SIZE,
                                         PJMEDIA_STREAM_INC);
  ...
  *p++ = 'g';
  ...
  if (status != PJ_SUCCESS) goto err_cleanup;
  ...
  if (info->param)
    ...
    status = pjmedia_codec_mgr_get_default_param(stream->codec_mgr, &info->fmt,
                                                 &stream->codec_param);
    if (status != PJ_SUCCESS) goto err_cleanup;
  }
  ...
  if (pj_stricmp(&info->fmt.encoding_name, &opus_tag) == 0) {
    ...
    stream->rtp_opus_tx_ts_len_per_pkt =
        stream->enc_samples_per_pkt * 48000 /
  ...
  status = create_channel(pool, stream, PJMEDIA_DIR_DECODING, info->rx_pt, info,
                          &stream->dec);
  if (status != PJ_SUCCESS) goto err_cleanup;

  /* Create encoder channel: */

  status = create_channel(pool, stream, PJMEDIA_DIR_ENCODING, info->tx_pt, info,
                          &stream->enc);
  if (status != PJ_SUCCESS) goto err_cleanup;
  ...
  status = pjmedia_transport_attach(
      tp, stream, &info->rem_addr, &info->rem_rtcp,
      pj_sockaddr_get_len(&info->rem_addr), &on_rx_rtp, &on_rx_rtcp);
  if (status != PJ_SUCCESS) goto err_cleanup;
  ...
  if (info->rtcp_xr_enabled) {
    ...
    if (info->rtcp_xr_interval != 0)
      ...
      stream->rtcp_xr_interval = (PJMEDIA_RTCP_INTERVAL + (pj_rand() % 8000)) *
                                 info->fmt.clock_rate / 1000;
  ...
  if (stream->use_ka) send_keep_alive_packet(stream);
#endif
...
#if TRACE_JB
  {
    ...
    pj_ansi_snprintf(trace_name, sizeof(trace_name),
                     TRACE_JB_PATH_PREFIX "%s.csv", stream->port.info.name.ptr);
    status = pj_file_open(pool, trace_name, PJ_O_WRONLY, &stream->trace_jb_fd);
    if (status != PJ_SUCCESS) {
      stream->trace_jb_fd = TRACE_JB_INVALID_FD;
      PJ_LOG(3, (THIS_FILE, "Failed creating RTP trace file '%s'", trace_name));
    } else {
      ...
      if (len < 1 || len >= PJ_LOG_MAX_SIZE) len = PJ_LOG_MAX_SIZE - 1;
      pj_file_write(stream->trace_jb_fd, stream->trace_jb_buf, &len);
      pj_file_flush(stream->trace_jb_fd);
    }
  }
...
PJ_DEF(pj_status_t) pjmedia_stream_destroy(pjmedia_stream *stream) {
  ...
  if (stream->tx_dtmf_count && stream->tx_dtmf_buf[0].duration != 0) {
     ...
     *  - RTP timestamp is constant for the same packet.
     */
    status = pjmedia_rtp_encode_rtp(&channel->rtp, stream->tx_event_pt, first,
  ...
  if (stream->jb_mutex) pj_mutex_lock(stream->jb_mutex);
  ...
  if (stream->jb) pjmedia_jbuf_destroy(stream->jb);
...
/*
 * Vlad - Modify codec param
 */
PJ_DEF(pj_status_t)
pjmedia_stream_codec_modify(pjmedia_stream *st,
                            const pjmedia_codec_param *param) {
  return pjmedia_codec_modify(st->codec, param);
}
...
PJ_DEF(pj_status_t) pjmedia_stream_get_dtmf(pjmedia_stream *stream,
  ...
  if (stream->rx_dtmf_count < *size) *size = stream->rx_dtmf_count;
